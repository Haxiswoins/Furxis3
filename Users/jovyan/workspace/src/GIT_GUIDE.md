...
# 2. 为这些修改创建一个提交（"存档点"）
git commit -m "在服务器上做了一些临时修改"
```

**第二步：拉取并尝试合并 GitHub 的代码**
```bash
# 尝试拉取并合并 main 分支的更改
# 如果您想从 3.2 分支拉取，就把 "main" 换成 "3.2"
git pull origin main
```

*   如果此命令成功执行且没有提示 `CONFLICT` (冲突)，那么恭喜您，合并很顺利。
*   如果此命令提示 `CONFLICT`，您**必须**在服务器上手动解决这些冲突文件，然后再次 `add` 和 `commit`。**这是一个高风险操作，强烈不建议在生产服务器上进行。**
