...
export function ContentClientPage({ initialContent }: ContentClientPageProps) {
    const { toast } = useToast();
    const router = useRouter();
    const [loading, setLoading] = useState(false);
    const [initialLoading, setInitialLoading] = useState(!initialContent);
    ...
    const form = useForm<FormValues>({
        ...
        defaultValues: {
            commissionTitle: initialContent?.commissionTitle || '',
            commissionDescription: initialContent?.commissionDescription || '',
            adoptionTitle: initialContent?.adoptionTitle || '',
            adoptionDescription: initialContent?.adoptionDescription || '',
            workTitle: initialContent?.workTitle || '',
            workDescription: initialContent?.workDescription || '',
            adoptionPageDescription: initialContent?.adoptionPageDescription || '',
            commissionPageDescription: initialContent?.commissionPageDescription || '',
            adminEmail: initialContent?.adminEmail || '',
            senderEmail: initialContent?.senderEmail || '',
            sunriseHour: initialContent?.sunriseHour ?? 6,
            sunsetHour: initialContent?.sunsetHour ?? 18,
            contactInfo: initialContent?.contactInfo || '',
            fanPrice: initialContent?.fanPrice ?? 150,
            magneticEyePrice: initialContent?.magneticEyePrice ?? 200,
            commissionImageUrl: initialContent?.commissionImageUrl || '',
            adoptionImageUrl: initialContent?.adoptionImageUrl || '',
            workImageUrl: initialContent?.workImageUrl || '',
        },
    });
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>, setFile: React.Dispatch<React.SetStateAction<File | null>>, urlField: keyof FormValues) => {
        const file = e.target.files?.[0];
        if (file) {
            setFile(file);
            form.setValue(urlField, URL.createObjectURL(file));
        }
    };
    ...
    const handleSave = async (values: FormValues) => {
        ...
        try {
            ...
            let newCommissionImageUrl = values.commissionImageUrl;
            let newAdoptionImageUrl = values.adoptionImageUrl;
            let newWorkImageUrl = values.workImageUrl;
                ...
                <form onSubmit={form.handleSubmit(handleSave)} className="space-y-8">
                     <Card>
                        <CardHeader>
                           ...
                           <CardDescription>配置网站的核心参数。</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-6">
                             <FormField
                                    ...
                                    <FormLabel>首页卡片背景图</FormLabel>
                                     <div className="flex items-start gap-4">
                                        <div className="w-32 h-48 relative rounded-md border bg-muted flex-shrink-0">
                                            {commissionUrlValue && <Image src={commissionUrlValue} alt="委托卡片预览" fill style={{objectFit:'cover'}} className="rounded-md"/>}
                                        </div>
                                        <div className="space-y-2">
                                            ...
                                            <Input
                                                ...
                                                type="file"
                                                accept="image/*"
                                                onChange={(e) => handleFileChange(e, setCommissionImageFile, 'commissionImageUrl')}
                             ...
                             <CardContent className="space-y-6">
                                ...
                                <FormField control={form.control} name="adoptionDescription" render={({ field }) => ( <FormItem> <FormLabel>首页卡片描述</FormLabel> <FormControl><Input {...field} /></FormControl> <FormMessage /> </FormItem> )}/>
                                 ...
                                 <div className="space-y-2">
                                    ...
                                    <div className="flex items-start gap-4">
                                        <div className="w-32 h-48 relative rounded-md border bg-muted flex-shrink-0">
                                            { adoptionUrlValue && <Image src={adoptionUrlValue} alt="领养卡片预览" fill style={{objectFit:'cover'}} className="rounded-md" /> }
                                        </div>
                                        <div className="space-y-2">
                                            ...
                                            <Input
                                                ...
                                                type="file"
                                                accept="image/*"
                                                onChange={(e) => handleFileChange(e, setAdoptionImageFile, 'adoptionImageUrl')}
                             ...
                             <CardContent className="space-y-6">
                                ...
                                <FormField control={form.control} name="workDescription" render={({ field }) => ( <FormItem> <FormLabel>首页卡片描述</FormLabel> <FormControl><Input {...field} /></FormControl> <FormMessage /> </FormItem> )}/>
                                 <div className="space-y-2">
                                    ...
                                    <div className="flex items-start gap-4">
                                        <div className="w-32 h-48 relative rounded-md border bg-muted flex-shrink-0">
                                            { workUrlValue && <Image src={workUrlValue} alt="作品卡片预览" fill style={{objectFit:'cover'}} className="rounded-md" /> }
                                        </div>
                                        <div className="space-y-2">
                                            ...
                                            <Input
                                                ...
                                                type="file"
                                                accept="image/*"
                                                onChange={(e) => handleFileChange(e, setWorkImageFile, 'workImageUrl')}
