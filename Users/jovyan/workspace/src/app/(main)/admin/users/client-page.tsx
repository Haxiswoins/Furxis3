...
function GrantBadgeDialog({ user, badges, onBadgeGranted }: GrantBadgeDialogProps) {
...
type UserManagementPageClientProps = {
  initialUsers: AggregatedUser[];
  initialBadges: Badge[];
};

export function UserManagementPageClient({ initialUsers, initialBadges }: UserManagementPageClientProps) {
  const { toast } = useToast();
  const [users, setUsers] = useState<AggregatedUser[]>(initialUsers);
  ...
  const fetchData = useCallback(async () => {
    try {
      const userData = await getAggregatedUsers();
      setUsers(userData);
    } catch (error) {
      toast({
        title: '刷新数据失败',
        description: '无法获取最新的用户列表。',
        variant: 'destructive',
      });
    }
  }, [toast]);
