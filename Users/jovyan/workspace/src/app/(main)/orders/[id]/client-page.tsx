...
import { getSecureOrderById, reinstateOrder, getSiteContent, updateOrder, cancelOrder } from '@/lib/data-service';
...
export default function OrderDetailPage() {
  ...
  const fetchOrderAndContent = useCallback(async () => {
    ...
    try {
      // Use the new secure function for fetching data
      const [orderData, contentData] = await Promise.all([
        getSecureOrderById(orderId, user.uid),
        getSiteContent()
      ]);
      ...
      if (orderData) {
        ...
        // If getSecureOrderById returns null, it means not found or not authorized.
        notFound();
      }
    } catch (error) {
