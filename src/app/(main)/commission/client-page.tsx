      ...
      {commissionOptions.length > 0 ? (
        commissionOptions.map((item) => (
          <motion.div key={item.id} variants={itemVariants}>
            <Link href={`/commission/${encodeURIComponent(item.name)}`} className="group block relative aspect-video md:aspect-[16/4] rounded-xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-1">
              <Image
                src={item.imageUrl}
                alt={item.name}
                fill
              ...
              />
              <div className="absolute inset-0 flex flex-row items-center p-4 md:p-8 text-white bg-gradient-to-r from-black/80 via-black/50 to-transparent">
                <div className="w-full md:w-2/3 space-y-2">
                  ...
                  <p className="text-xs opacity-80 mt-2 line-clamp-2 md:line-clamp-3" style={{ textShadow: '1px 1px 3px rgba(0,0,0,0.7)' }}>{item.description}</p>
                </div>
