...
export default function OrdersPage() {
  ...
  useEffect(() => {
    // This function will handle fetching the orders.
    ...
    // We wait for the authentication to be finalized.
    if (!authLoading) {
      // If authentication is done and there's a user with a UID, fetch their orders.
      if (user && user.uid) {
        fetchOrders(user.uid);
      } else {
        // If there's no user, redirect to login and stop loading.
        router.push('/login');
        setLoading(false);
      }
    }
  }, [user, user?.uid, authLoading, router]); // Depend explicitly on user.uid
